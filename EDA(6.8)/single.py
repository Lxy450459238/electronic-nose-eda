...
class SingleFileEDA:
    ...
    def analyze_baseline(self):
        ...
        if bad_outlier_sensors:
            ...
            self.quality_score -= Config.SCORE_BASELINE_OUTLIERS  # 毛刺超标一次性扣分（不按传感器累计）
    ...
    def analyze_cleaning(self):
        ...
        crossed_baseline_mask = np.zeros(self.num_sensors, dtype=bool)  # 反向越界标记
        over_recovery_count = 0
        overclean_sensors = []   # 过度清洗（越界但残留率≤0.5）
        drift_warning = []       # 基线漂移（越界且残留率>0.5）
        ...
        for sensor_idx in eval_sensors:
            ...
            # 残留率统一取绝对值，恒≥0（数学定义自洽）
            recovery_rate[sensor_idx] = abs(resid) / abs(resp_amp)

            # 检测反向越界：响应方向与残留方向相反（清洗越过基线）
            if resp_amp * resid < 0:
                crossed_baseline_mask[sensor_idx] = True
                over_recovery_count += 1
                if recovery_rate[sensor_idx] > 0.5:
                    drift_warning.append(sensor_idx)
                else:
                    overclean_sensors.append(sensor_idx)

        if over_recovery_count > 0:
            print(f"\n  [INFO] {over_recovery_count}个传感器反向越界（越过基线）")
            if len(overclean_sensors) > 0:
                overclean_names = [Config.get_sensor_name(int(idx), self.version_config) for idx in overclean_sensors]
                print(f"    过度清洗 ({len(overclean_sensors)}个，残留率≤50%): {overclean_names[:5]}")
            if len(drift_warning) > 0:
                drift_names = [Config.get_sensor_name(int(idx), self.version_config) for idx in drift_warning]
                print(f"    基线漂移 ({len(drift_warning)}个，残留率>50%): {drift_names[:5]}")
        ...
        cleaning_degree = (1 - avg_recovery) * 100

        print(f"\n  {clean_stage}平均清洗度: {cleaning_degree:.1f}%")
        if avg_recovery > 1.0:
            print(f"    [INFO] 平均残留率>100%，传感器在清洗阶段可能发生漂移而非单纯未恢复")

        # 优良率统计：排除反向越界传感器（方案A：不计分子分母）
        eval_array = np.array(eval_sensors)
        crossed_eval = crossed_baseline_mask[eval_array]
        normal_eval = eval_array[~crossed_eval]  # 未越界的正常传感器

        if len(normal_eval) > 0:
            normal_recovery = recovery_rate[normal_eval]
            excellent = np.sum(normal_recovery < 0.05)
            good = np.sum((normal_recovery >= 0.05) & (normal_recovery < 0.15))
            poor = np.sum(normal_recovery >= 0.25)
        ...
        threshold = max(1, int(len(eval_sensors) * 0.8))  # 基于总评估传感器数计算阈值

        if normal_count > 0 and excellent + good >= threshold:
            print(f"    [OK] 清洗充分 (优秀:{excellent}, 良好:{good}, 基于{normal_count}个正常传感器)")
            self.quality_score += 10
        ...
        # 反向越界传感器单独报告
        if over_recovery_count > 0:
            crossed_ratio = over_recovery_count / len(eval_sensors)
            print(f"\n  [反向越界传感器] {over_recovery_count}个 (占评估传感器 {crossed_ratio*100:.1f}%)")
            crossed_indices = eval_array[crossed_eval]
            for idx in crossed_indices[:5]:
                real_name = Config.get_sensor_name(idx, self.version_config)
                tag = "基线漂移" if idx in drift_warning else "过度清洗"
                print(f"      {idx} ({real_name}): 残留率{recovery_rate[idx]*100:.1f}% [{tag}]")
            # 反向越界超标提醒（暂不扣分）
            if crossed_ratio > Config.CROSSED_BASELINE_MAX_RATIO:
                self.warnings.append(
                    f"[WARNING] {clean_stage}反向越界传感器占比{crossed_ratio*100:.1f}%超过阈值"
                    f"{Config.CROSSED_BASELINE_MAX_RATIO*100:.0f}%，建议关注硬件状态")

        if poor > 0 and len(normal_eval) > 0:
            poor_mask = recovery_rate[normal_eval] >= 0.25
            poor_normal = normal_eval[poor_mask]
            if len(poor_normal) > 0:
                print(f"\n  [WARNING] {len(poor_normal)}个正常传感器恢复不足:")
                for idx in poor_normal[:3]:
                    real_name = Config.get_sensor_name(idx, self.version_config)
        ...
        self.results['crossed_baseline'] = {
            'count': over_recovery_count,
            'overclean_count': len(overclean_sensors),
            'drift_count': len(drift_warning),
            'overclean_sensors': overclean_sensors,
            'drift_sensors': drift_warning,
        }
    ...
    def save_text_report(self, save_path):
        ...
        with open(save_path, 'w', encoding='utf-8') as f:
            ...
            if 'cleaning_degree' in self.results:
                ...
                # 反向越界报告
                crossed_info = self.results.get('crossed_baseline', {})
                if crossed_info and crossed_info.get('count', 0) > 0:
                    crossed_count = crossed_info.get('count', 0)
                    overclean_count = crossed_info.get('overclean_count', 0)
                    drift_count = crossed_info.get('drift_count', 0)
                    f.write(f"\n  [反向越界传感器] 共{crossed_count}个越过基线（不纳入优良率统计）:\n")
                    f.write(f"    过度清洗: {overclean_count}个 (恢复方向反转，残留率≤50%)\n")
                    f.write(f"    基线漂移: {drift_count}个 (残留率>50%，可能存在硬件不稳定性)\n")

                if cd < 0:
                    f.write(f"  [INFO] 平均残留率超过100%，可能存在传感器漂移\n")
                elif cd >= 80:
                    f.write(f"  评价: 清洗充分\n")
                elif cd >= 60:
                    f.write(f"  评价: 清洗可接受\n")
            ...
            if 'temporal_correlation' in self.results:
                ...
                high_pairs = corr_info.get('high_corr_pairs', [])
                sample_stage = self.results.get('sample_stage', '进样')
                f.write(f"\n--- 时间序列相关性分析 ---\n")
                f.write(f"  基于{sample_stage}阶段前半段动态响应数据\n")
                f.write(f"  高度相关传感器对（|r|>0.8）: {len(high_pairs)}对\n")
                if len(high_pairs) > 0:
                    sorted_pairs = sorted(high_pairs, key=lambda x: abs(x[2]), reverse=True)
                    top_n = min(15, len(sorted_pairs))
                    f.write(f"\n  相关系数绝对值最高的传感器对（前{top_n}）:\n")
                    for i, j, r in sorted_pairs[:top_n]:
                        name_i = Config.get_sensor_name(i, self.version_config)
                        name_j = Config.get_sensor_name(j, self.version_config)
                        f.write(f"    {name_i} <-> {name_j}: r={r:.3f}\n")
                f.write("\n")
